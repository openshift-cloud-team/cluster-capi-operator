...
func TestFindLatestRevision(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			latest := r.findLatestRevision(clusterAPI)
...
func TestBuildConditions(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			result:                    reconcileResult{reason: conditionReasonNonRetryableError, error: errMaxRevisionsAllowed},
			expectedProgressingStatus: configv1.ConditionFalse,
		...
		},
		{
			...
			result:                    reconcileResult{reason: conditionReasonEphemeralError, error: errors.New("connection refused")},
		...
		},
		{
			...
			result:                    reconcileResult{reason: conditionReasonWaitingOnExternal, message: "Infrastructure not found"},
			expectedProgressingStatus: configv1.ConditionTrue,
