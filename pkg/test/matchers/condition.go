...
func findCondition(conditionSlice reflect.Value, conditionType string) (reflect.Value, bool, error) {
	for i := 0; i < conditionSlice.Len(); i++ {
		...
		if !typeField.IsValid() {
			return reflect.Value{}, false, fmt.Errorf("condition element at index %d does not have a Type field", i)
		}
...
func (m *ConditionMatcher) Match(actual interface{}) (bool, error) {
	...
	for _, matchField := range []struct {
		...
		{matcher: m.statusMatcher, name: "Status"},
		{matcher: m.reasonMatcher, name: "Reason"},
		{matcher: m.messageMatcher, name: "Message"},
		{matcher: m.lastTransitionMatcher, name: "LastTransitionTime"},
	} {
